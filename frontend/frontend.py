...
def draw_gradio_ui(opt, img2img=lambda x: x, txt2img=lambda x: x, imgproc=lambda x: x, txt2img_defaults={}, RealESRGAN=True, GFPGAN=True, LDSR=True,
    ...
    with gr.Blocks(css=css(opt), analytics_enabled=False, title="Stable Diffusion WebUI") as demo:
        with gr.Tabs(elem_id='tabss') as tabs:
            with gr.TabItem("Text-to-Image", id='txt2img_tab'):
                    ...
                    with gr.Column():
                        with gr.Box():
                            output_txt2img_gallery = gr.Gallery(
                                label="Images", elem_id="txt2img_gallery_output").style(grid=[4, 4])
                            gr.Markdown(
                        ...
                        output_txt2img_params = gr.Highlightedtext(
                            label="Generation parameters", interactive=False, elem_id='highlight')
                        with gr.Group():
                            with gr.Row(elem_id='txt2img_output_row'):
                                ...
                                input_txt2img_paste_parameters = gr.Button(
                                    'Paste settings', visible=pyperclip is not None)
                                input_txt2img_from_file = gr.Button(
                                    'From file...')
                                input_txt2img_defaults = gr.Button(
                                    'Restore defaults')
                        ...
                        with gr.Tabs():
                            with gr.TabItem('Simple'):
                                ...
                                txt2img_submit_on_enter.change(
                                    lambda x: gr.update(
                                        max_lines=1 if x == 'Yes' else 25), txt2img_submit_on_enter,
                                    txt2img_prompt)
                ...
                txt2img_width.change(fn=uifn.update_dimensions_info, inputs=[
                                     txt2img_width, txt2img_height], outputs=txt2img_dimensions_info_text_box)
                txt2img_height.change(fn=uifn.update_dimensions_info, inputs=[
                                      txt2img_width, txt2img_height], outputs=txt2img_dimensions_info_text_box)
                ...
                txt2img_settings_checkboxgroup_info = [
                    (3, txt2img_toggles.choices)]
                ...
                live_prompt_params = [txt2img_prompt, txt2img_width, txt2img_height,
                                      txt2img_steps, txt2img_seed, txt2img_batch_count, txt2img_cfg]
            ...
            with gr.TabItem("Image-to-Image Unified", id="img2img_tab"):
                    ...
                    with gr.Column():
                        ...
                        with gr.Tabs():
                            with gr.TabItem("Editor Options"):
                                ...
                                img2img_painterro_btn = gr.Button(
                                    "Advanced Editor")
                            with gr.TabItem("Hints"):
                                img2img_help = gr.Markdown(
                                    visible=False, value=uifn.help_text)

                    with gr.Column():
                        gr.Markdown('#### Img2Img Results')
                        output_img2img_gallery = gr.Gallery(
                            label="Images", elem_id="img2img_gallery_output").style(grid=[4, 4, 4])
                        ...
                        with gr.Tabs():
                            ...
                            with gr.TabItem("Output info", id="img2img_output_info_tab"):
                                output_img2img_params = gr.Textbox(
                ...
                img2img_image_editor_mode.change(
                    ...
                    [img2img_image_editor_mode, img2img_image_editor,
                        img2img_resize, img2img_width, img2img_height],
                ...
                img2img_image_editor.edit(
                    ...
                    [img2img_image_editor, img2img_resize,
                ...
                img2img_inputs = [img2img_prompt, img2img_image_editor_mode, img2img_image_editor, img2img_mask,
                                  ...
                                  img2img_embeddings]
                img2img_outputs = [output_img2img_gallery, output_img2img_seed,
                                   output_img2img_params, output_img2img_stats]
                ...
                img2img_painterro_btn.click(None,
                                            ...
                                            [img2img_image_editor,
                                                img2img_image_mask],
                                            _js=call_JS(
                                                "Painterro.init", toId="img2img_editor")
                                            )

                img2img_width.change(fn=uifn.update_dimensions_info, inputs=[
                                     img2img_width, img2img_height], outputs=img2img_dimensions_info_text_box)
                img2img_height.change(fn=uifn.update_dimensions_info, inputs=[
                                      img2img_width, img2img_height], outputs=img2img_dimensions_info_text_box)
                        ...
                        # select folder with images to process
                            with gr.TabItem('Batch Process'):
                                imgproc_folder = gr.File(
                                    label="Batch Process", file_count="multiple", source="upload", interactive=True, type="file")
                        imgproc_pngnfo = gr.Textbox(
                            label="PNG Metadata", placeholder="PngNfo", visible=False, max_lines=5)
                        with gr.Row():
                            imgproc_btn = gr.Button(
    ...
    """)
                        ...
                        with gr.Box(visible=False) as upscale_group:
                            ...
                            if LDSR:
                                upscaleModes = [
                                    'RealESRGAN', 'GoBig', 'Latent Diffusion SR', 'GoLatent ']
                            else:
                                gr.HTML("""
    <div id="90" style="max-width: 100%; font-size: 14px; text-align: center;" class="output-markdown gr-prose border-solid border border-gray-200 rounded gr-panel">
    ...
    """)
                            ...
                            with gr.Column():
                                ...
                                imgproc_upscale_toggles = gr.Radio(
                                    label='', choices=upscaleModes, type="index", visible=RealESRGAN is not None, value='RealESRGAN')
        ...
        """)
            imgproc_toggles.change(fn=uifn.toggle_options_gfpgan, inputs=[
                                   imgproc_toggles], outputs=[gfpgan_group])
            imgproc_toggles.change(fn=uifn.toggle_options_upscalers, inputs=[
                                   imgproc_toggles], outputs=[upscale_group])
            imgproc_toggles.change(fn=uifn.toggle_options_upscalers, inputs=[
                                   imgproc_toggles], outputs=[upscalerSettings_group])
            imgproc_upscale_toggles.change(fn=uifn.toggle_options_realesrgan, inputs=[
                                           imgproc_upscale_toggles], outputs=[realesrgan_group])
            imgproc_upscale_toggles.change(fn=uifn.toggle_options_ldsr, inputs=[
                                           imgproc_upscale_toggles], outputs=[ldsr_group])
            imgproc_upscale_toggles.change(fn=uifn.toggle_options_gobig, inputs=[
                                           imgproc_upscale_toggles], outputs=[gobig_group])
    ...
    """)
        ...
        load_detector = gr.Number(
            value=0, label="Load Detector", visible=False)
